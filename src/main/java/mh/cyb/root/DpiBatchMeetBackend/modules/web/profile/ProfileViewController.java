...
public class ProfileViewController {
        ...
        public String viewMyProfile(@AuthenticationPrincipal UserDetails userDetails, Model model) {
                ...
                return viewProfile(user.getId(), userDetails, model);
        }
        ...
        public String viewProfile(@PathVariable Long id,
                ...
                model.addAttribute("profile", profileService.getProfile(targetUser));
                model.addAttribute("isOwner", isOwner);

                if (!isOwner) {
                        PrivacySetting privacy = privacySettingRepository.findByUserId(targetUser.getId())
                                        .orElseGet(() -> PrivacySetting.builder().user(targetUser).build());
